...
session_state = SessionState.SessionState.get(user_name='', passwd='')
def app():
	st.title("Tweet Analyzer")
	activities=["Login","Sign up","Tweet Analyzer","Generate Twitter Data"]
	choice = st.sidebar.selectbox("Select Your Activity",activities)
	
	#session_state = SessionState(user_name='', passwd='')
	
	def check_state():
		#global session_state
		session_state = SessionState.SessionState.get(user_name='', passwd='')
		user_name = session_state.user_name
		passwd = session_state.passwd
		if check(user_name,passwd)=='success':
			return 'pass'

	if choice=="Login":
		st.header("Welcome to login page for tweet analyser")
		st.subheader("to proceed further please login or signup")
		...
		if st.button("login"):
			if check(username,password)=='success':
				st.write('login successful, you can now access other tabs')
				global session_state
				session_state = SessionState.SessionState.get(user_name='', passwd='')
				session_state.user_name=username
				session_state.passwd=password
			else:
				st.write('incorrect credentials')

	elif choice=="Sign up":
		
		st.header("Welcome to sign up page for tweet analyser")
		st.subheader("to proceed further please login or signup")
		username = st.text_input("please enter username")
		...
		if st.button("Sign up"):
			if signup(username,email,password)=='success':
		...
		#if check_state!='pass':
		#	st.header(session_state.user_name)
		#	st.header(session_state.passwd)
		#	st.header("Please login to access this functionality")
		if check(user_name,passwd)=='success':
